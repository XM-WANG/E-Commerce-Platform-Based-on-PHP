...
$fp = fsockopen('ssl://www.sandbox.paypal.com', 443, $errno, $errstr, 30);
 ...
 while (!feof($fp)) {
	...
	if (strcmp ($res, "VERIFIED\r\n") == 0) { 
		...
		*/
		$email="1155121439-facilitator@link.cuhk.edu.hk";
		...
		do{
			...
			$db = ierg4210_DB();
			$sql = $db->prepare("SELECT name FROM products WHERE pid = ?");
			$sql-> execute(array($_POST['item_number'.$i]));
			if($r = $sql -> fetch()){
				$name = $r['name'];
			}
			array_push($nameList, $name);
			$shoppingcart_info.=$name.'&'.$_POST['mc_gross_'.$i].'&'.$_POST['quantity'.$i].'|';
		...
		*/
		...
		$userEmail = $t['em'];
		$digest = sha1($Currency.$userEmail.$salt.$shoppingcart_info.$totalPrice);

        if ($digest==$digestOld)
		{	
			$q = $db->prepare("UPDATE orders SET tranid = ? WHERE oid = ?");
